 ...
 * 
 * 
 * Github: https://github.com/shaibrown5/Wattson
 *
*/

/**  code for wattson plug.
* 	this code read the Irms of the corrent item 
*	calc the watt power and send it via wifi to firebase DB
*/
...
/* Define the Firebase project host name (required) */
#define FIREBASE_HOST "******your firebasr hostname*****"
...
/**  Define the Service Account credentials (required for token generation)
 * 
...
#define FIREBASE_CLIENT_EMAIL "******firebase client_email"
const char PRIVATE_KEY[] PROGMEM ="your key example below";
...
int count = 0;
EnergyMonitor emon1;                   // Create an instance for IRMS calc
...
void setup()
{
    ...
    bool res;
    res = wm.autoConnect("Wattson"); // anonymous ap
    //res = wm.autoConnect("Wattson","password"); // password protected ap

    while(!res) {
      
        Serial.println("Failed to connect");
        res = wm.autoConnect("Wattson");
    } 
...
{	
    ...
    if (millis() - dataMillis > 1000)
    {
        ...
        double power =Irms*230.0*0.8;
        Serial.println(Irms);
        Serial.println(power);
        Serial.println("sending ...");

        if(Irms < 1.5) {
          sendDataToFirebase(0,count++);
        }
        else{
          sendDataToFirebase(power,count++);
        }
...
void sendDataToFirebase(double power,int idx){
  /* Get the token status */
  int timestamp = 0;
  TokenInfo info = Firebase.authTokenInfo();
        if (info.status == token_status_ready)
        {
            if (Firebase.setTimestamp(fbdo, path + "/temp" ))
            {
            Serial.println("PASSED");
            Serial.println(fbdo.intData());
            timestamp = fbdo.intData();
          ...
          if (Firebase.deleteNode(fbdo,path + "/temp")){
          ...
          else
          {
            ...
            if (Firebase.set(fbdo, path + "/" + String(timestamp) , power))
            {
                Serial.println(timestamp);
                Serial.println("PASSED");
